...
Require Import DG4iSL_adm_ImpL.
Require Import DG4iSL_inv_ImpR.


Theorem ImpImpL_inv_L :  forall n s (D0 : derrec G4iSL_rules (fun _ => False) s) A B C D Γ0 Γ1,
                              (n = derrec_height D0) ->
                              (s = (Γ0 ++ (A  → B) → D :: Γ1, C)) ->
                              derrec G4iSL_rules (fun _ => False) (Γ0 ++ A :: B → D :: B → D :: Γ1, C).
Proof.
assert (DersNilF: dersrec G4iSL_rules (fun _ : Seq  => False) []).
apply dersrec_nil.
(* Setting up the strong induction on the height. *)
pose (strong_inductionT (fun (x:nat) => forall s (D0 : derrec G4iSL_rules (fun _ => False) s) A B C D Γ0 Γ1,
                              (x = derrec_height D0) ->
                              (s = (Γ0 ++ (A  → B) → D :: Γ1, C)) ->
                              derrec G4iSL_rules (fun _ => False) (Γ0 ++ A :: B → D :: B → D :: Γ1, C))).
