...
pose (strong_inductionT (fun (x:nat) => forall (concl : Seq)
  (D0 : derrec G4iSL_rules (fun _ : Seq => False) concl), x = derrec_height D0 ->
((forall prem, ((AndLRule [prem] concl) -> existsT2 (D1 : derrec G4iSL_rules (fun _ => False) prem),
 derrec_height D1 <= x))) * ((forall prem1 prem2, ((AndRRule [prem1; prem2] concl) ->
 existsT2 (D1 : derrec G4iSL_rules (fun _ => False) prem1) (D2 : derrec G4iSL_rules (fun _ => False) prem2),
...
{ intros prem RA. inversion RA. subst.
  ...
  * inversion H. subst. assert (InT (Bot) (Γ0 ++ And A B :: Γ1)). rewrite <- H2. apply InT_or_app. right. apply InT_eq.
    ...
    pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[]) (x ++ Bot :: x0, C) H1 DersNilF). exists d0.
    simpl. rewrite dersrec_height_nil. lia. reflexivity.
   (* AndR *)
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
   ...
   (Γ0 ++ A :: B :: Γ1, And A0 B0)). apply AndRRule_I. pose (dlCons x1 DersNilF). pose (dlCons x2 d0). apply AndR in H0.
    pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ0 ++ A :: B :: Γ1, A0); (Γ0 ++ A :: B :: Γ1, B0)])
  ...
  * inversion H ; subst. apply app2_find_hole in H2. destruct H2. repeat destruct s ; destruct p ; subst.
   ...
   + destruct x.
      ...
      { inversion e0.  subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
        ...
        pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ2 ++ A0 :: B0 :: x ++ A :: B :: Γ1, C)])
    ...
    + destruct x.
       ...
       { inversion e0. subst. clear e0. assert (J30: dersrec_height d = dersrec_height d). reflexivity. simpl.
         ...
         pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[((Γ0 ++ A :: B :: x) ++ A0 :: B0 :: Γ3, C)])
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ0 ++ A :: B :: Γ1, A0)])
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ0 ++ A :: B :: Γ1, B0)])
    (Γ0 ++ A :: B :: Γ1, Or A0 B0) H0 d0). exists d1. simpl. rewrite dersrec_height_nil. lia. reflexivity.
  (* OrL *)
  * inversion H. subst. apply app2_find_hole in H2. destruct H2. repeat destruct s ; destruct p ; subst.
   + inversion e0.
   + destruct x.
      ...
      { inversion e0. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
        ...
        pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ2 ++ A0 :: x ++ A :: B :: Γ1, C); (Γ2 ++ B0 :: x ++ A :: B :: Γ1, C)])
    ...
    + destruct x.
       ...
       { inversion e0. subst. clear e0. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
          ...
          pose (IH _ J8 _ _ J9). destruct p. clear s0. pose (s _ J7). destruct s0. clear s.
          pose (dlCons x3 DersNilF). pose (dlCons x2 d0).
          pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ0 ++ A :: B :: x ++ A0 :: Γ3, C); (Γ0 ++ A :: B :: x ++ B0 :: Γ3, C)])
  ...
  * inversion H ; subst. simpl in IH. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    pose (G4iSL_hpadm_list_exch_L _ _ x J31 _ l). destruct s. pose (ImpRRule_I A0 B0 [] (Γ0 ++ A :: B :: Γ1)). simpl in i.
   ...
   + destruct x.
      ...
      { inversion e0.  subst. apply app2_find_hole in H2. destruct H2. repeat destruct s ; destruct p ; subst.
        ...
        - destruct x0. simpl in e1. inversion e1. inversion e1. subst. clear e1. clear e0.
          ...
          pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ2 ++ # P :: Γ3 ++ A0 :: x0 ++ A :: B :: Γ1, C)])
        ...
        - destruct x0. simpl in e1. inversion e1. inversion e1. subst. simpl.
          ...
          pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ2 ++ # P :: x ++ A :: B :: x0 ++ A0 :: Γ4, C)])
    ...
    + destruct x. simpl in e0. inversion e0. inversion e0. subst. clear e0.
       ...
       repeat rewrite <- app_assoc. auto. rewrite H1 in H0.
       pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[((Γ0 ++ A :: B :: x) ++ # P :: Γ3 ++ A0 :: Γ4, C)])
   ...
   + destruct x.
      ...
      { inversion e0.  subst. apply app2_find_hole in H2. destruct H2. repeat destruct s ; destruct p ; subst.
        ...
        - destruct x0. simpl in e1. inversion e1. inversion e1. subst. clear e1. clear e0.
          ...
          pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ2 ++ A0 :: Γ3 ++ # P :: x0 ++ A :: B :: Γ1, C)])
        ...
        - destruct x0. simpl in e1. inversion e1. inversion e1. subst. simpl.
          ...
          pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ2 ++ A0 :: x ++ A :: B :: x0 ++ # P :: Γ4, C)])
    ...
    + destruct x. simpl in e0. inversion e0. inversion e0. subst. clear e0.
       ...
       repeat rewrite <- app_assoc. auto. rewrite H1 in H0.
       pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[((Γ0 ++ A :: B :: x) ++ A0 :: Γ3 ++ # P :: Γ4, C)])
  ...
  * inversion H. subst. apply app2_find_hole in H2. destruct H2. repeat destruct s ; destruct p ; subst.
   ...
   + destruct x.
      ...
      { inversion e0.  subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
        ...
        pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ2 ++ Imp A0 (Imp B0 C0) :: x ++ A :: B :: Γ1, C)])
    ...
    + destruct x. simpl in e0. inversion e0. inversion e0. subst. clear e0.
      ...
      ((Γ0 ++ A :: B :: x) ++ Imp (And A0 B0) C0 :: Γ3, C)). apply AndImpLRule_I. repeat rewrite <- app_assoc in H0.
       ...
       repeat rewrite <- app_assoc. auto. rewrite H1 in H0.
       pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[((Γ0 ++ A :: B :: x) ++ Imp A0 (Imp B0 C0) :: Γ3, C)])
   ...
   + destruct x. { inversion e0. }
      { inversion e0.  subst. apply app2_find_hole in H2. destruct H2. repeat destruct s ; destruct p ; subst.
        - assert (J30: dersrec_height d = dersrec_height d). reflexivity.
          ...
          repeat rewrite <- app_assoc in H0. simpl in H0.  pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False)
        ...
        - assert (J30: dersrec_height d = dersrec_height d). reflexivity.
          ...
          pose (IH _ J5 _ _ J6). destruct p. clear s0. pose (s _ J4). destruct s0. clear s.
          pose (dlCons x1 DersNilF). apply OrImpL in H0. repeat rewrite <- app_assoc. simpl. repeat rewrite <- app_assoc. simpl.
          pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ2 ++ Imp A0 C0 :: Γ3 ++ Imp B0 C0 :: x0 ++ A :: B :: Γ1, C)])
        ...
        - destruct x0.
            + simpl in e1. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
               ...
               pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ2 ++ Imp A0 C0 :: x ++ Imp B0 C0 :: A :: B :: Γ1, C)])
            ...
            +  inversion e1. subst. simpl. repeat rewrite <- app_assoc. simpl. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
                ...
                pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ2 ++ Imp A0 C0 :: x ++ A :: B :: x0 ++ Imp B0 C0 :: Γ4, C)])
    ...
    + destruct x. simpl in e0. inversion e0. inversion e0. subst. clear e0. assert (J30: dersrec_height d = dersrec_height d). reflexivity. simpl.
       ...
       pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ0 ++ A :: B :: x ++ Imp A0 C0 :: Γ3 ++ Imp B0 C0 :: Γ4, C)])
 ...
 * inversion H. subst. apply app2_find_hole in H2. destruct H2. repeat destruct s ; destruct p ; subst.
   ...
   + destruct x. { inversion e0. }
      { inversion e0. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
        ...
        pose (dlCons x3 DersNilF). pose (dlCons x2 d0). pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False)
    ...
    + destruct x. { simpl in e0. inversion e0. }
       { inversion e0. subst. clear e0. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
          ...
          pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ0 ++ A :: B :: x ++ Imp B0 C0 :: Γ3, Imp A0 B0); (Γ0 ++ A :: B :: x ++ C0 :: Γ3, C)])
...
{ intros prem1 prem2 RA. inversion RA. subst.
  ...
  * inversion H. subst. simpl.
    ...
    pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[]) (Γ0 ++ Bot :: Γ1, A) H0 DersNilF). exists d0.
  ...
  * inversion H. subst. simpl. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ0 ++ A0 :: B0 :: Γ1, A)]) (Γ0 ++ And A0 B0 :: Γ1, A) H0 d0). exists d1.
    pose (dlCons x1 DersNilF). pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False)
  ...
  * inversion H. subst. simpl. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ0 ++ A0 :: Γ1, A); (Γ0 ++ B0 :: Γ1, A)]) (Γ0 ++ Or A0 B0 :: Γ1, A) H0 d1). exists d2.
    pose (dlCons x4 DersNilF).  pose (dlCons x2 d3). pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False)
  ...
  * inversion H. subst. simpl. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    pose (derI (rules:=G4iSL_rules) (prems:=fun _ : Seq => False) (ps:=[(Γ0 ++ # P :: Γ1 ++ A0 :: Γ2, B)]) (Γ0 ++ # P :: Γ1 ++ Imp # P A0 :: Γ2, B) H1 d2). exists d3.
    simpl. rewrite dersrec_height_nil. simpl. split. simpl. lia. lia. reflexivity.
  (* AtomImpL2 *)
  * inversion H. subst. simpl. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    pose (@dersrec_derrec_height (dersrec_height d) _ _ _ _ d J30). repeat destruct s.
...
Theorem AndL_inv : forall concl prem, (derrec G4iSL_rules (fun _ => False) concl) ->
                                      (AndLRule [prem] concl) ->
...
Theorem AndR_inv : forall concl prem1 prem2, (derrec G4iSL_rules (fun _ => False) concl) ->
                                      (AndRRule [prem1;prem2] concl) ->
                                      (derrec G4iSL_rules (fun _ => False) prem1) *
                                      (derrec G4iSL_rules (fun _ => False) prem2).
Proof.
intros.
assert (J1: derrec_height X = derrec_height X). reflexivity.
