...
Theorem AtomImpL_hpinv : forall n s (D0 : derrec G4iSL_rules (fun _ => False) s) P A Γ0 Γ1 C ,
                      (n = derrec_height D0) ->
                      (s = (Γ0 ++ (# P → A) :: Γ1, C)) ->
                      existsT2 (D1: (derrec G4iSL_rules (fun _ => False) (Γ0 ++ A :: Γ1, C))), (derrec_height D1 <= n).
Proof.
assert (DersNilF: dersrec G4iSL_rules (fun _ : Seq  => False) []).
apply dersrec_nil.
(* Setting up the strong induction on the height. *)
pose (strong_inductionT (fun (x:nat) => forall s (D0 : derrec G4iSL_rules (fun _ => False) s) P A Γ0 Γ1 C,
                      (x = derrec_height D0) ->
                      (s = (Γ0 ++ (# P → A) :: Γ1, C)) ->
                      existsT2 (D1: (derrec G4iSL_rules (fun _ => False) (Γ0 ++ A :: Γ1, C))), (derrec_height D1 <= x))).
...
Theorem AtomImpL_inv : forall P A Γ0 Γ1 C, (derrec G4iSL_rules (fun _ => False) (Γ0 ++ (# P → A) :: Γ1, C)) ->
