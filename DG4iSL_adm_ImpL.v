...
Theorem ImpL_adm : forall n s (D0:derrec G4iSL_rules (fun _ => False) s) A B Γ0 Γ1 C,
                                n = derrec_height D0 ->
                               (s = (Γ0 ++ Γ1, A)) ->
                               derrec G4iSL_rules (fun _ => False) (Γ0 ++ B :: Γ1, C) ->
                               derrec G4iSL_rules (fun _ => False) (Γ0 ++ Imp A B :: Γ1, C).
Proof.
...
pose (strong_inductionT (fun (x:nat) => forall (concl : Seq)
  (D0:derrec G4iSL_rules (fun _ => False) concl) A B Γ0 Γ1 C,
                                x = derrec_height D0 ->
                               (concl = (Γ0 ++ Γ1, A)) ->
                               derrec G4iSL_rules (fun _ => False) (Γ0 ++ B :: Γ1, C) ->
                               derrec G4iSL_rules (fun _ => False) (Γ0 ++ Imp A B :: Γ1, C))).
...
- intros A B Γ0 Γ1 C hei eq D1. subst.
  ...
  * inversion H ; subst.
    ...
    pose (G4iSL_hpadm_list_exch_L _ _ _ J1 _ J2). destruct s.
    apply G4iSL_adm_list_exch_L with (s:=(# P → B :: Γ2 ++ # P :: Γ3, C)).
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    apply G4iSL_adm_list_exch_L with (s:=(A → B :: Γ2 ++ A0 ∧ B0 :: Γ3, C)).
    ...
    pose (G4iSL_adm_list_exch_L _ D1 _ J1).
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    pose (@G4iSL_adm_wkn_L _ d0 _ _ J4). apply derI with (ps:=[(Γ0 ++ A0 → B :: B0 → B :: Γ1, C)]).
    apply OrImpL.
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    apply ImpImpLRule_I. apply dlCons. apply G4iSL_adm_wkn_L with (s:=(Γ0 ++ Γ1, A0 → B0)) (A:=B0 → B) ; auto.
    2: apply wkn_LI. apply derI with (ps:=[(Γ2 ++ A0 :: Γ3, B0)]). apply ImpR. rewrite <- H2. apply ImpRRule_I.
    apply dlCons ; auto. apply dlCons ; auto.
  (* AtomImpL1 *)
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    pose (@dersrec_derrec_height (dersrec_height d) _ _ _ _ d J30). repeat destruct s. simpl in IH.
    apply G4iSL_adm_list_exch_L with (s:=(A → B :: Γ2 ++ # P :: Γ3 ++ # P → A0 :: Γ4, C)).
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    pose (@dersrec_derrec_height (dersrec_height d) _ _ _ _ d J30). repeat destruct s. simpl in IH.
    apply G4iSL_adm_list_exch_L with (s:=(A → B :: Γ2 ++ # P → A0 :: Γ3 ++ # P :: Γ4, C)).
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    pose (@dersrec_derrec_height (dersrec_height d) _ _ _ _ d J30). repeat destruct s. simpl in IH.
    apply G4iSL_adm_list_exch_L with (s:=(A → B :: Γ2 ++ (A0 ∧ B0) → C0 :: Γ3, C)).
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    pose (@dersrec_derrec_height (dersrec_height d) _ _ _ _ d J30). repeat destruct s. simpl in IH.
    apply G4iSL_adm_list_exch_L with (s:=(A → B :: Γ2 ++ (A0 ∨ B0) → C0 :: Γ3 ++ Γ4, C)).
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    apply G4iSL_adm_wkn_L with (s:=(Γ2 ++ B0 → C0 :: Γ3, A0 → B0)) (A:=A → B) ; auto.
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    pose (@dersrec_derrec2_height (dersrec_height d) _ _ _ _ _ d J30). repeat destruct s. simpl in IH.
    apply G4iSL_adm_list_exch_L with (s:=(A → B :: Γ2 ++ Box A0 → B0 :: Γ3, C)).
    ...
    pose (wkn_LI (A → B) [] (unBoxed_list Γ2 ++ B0 :: unBoxed_list Γ3 ++ [Box A0]) A0). simpl in w. pose (G4iSL_adm_wkn_L x w). auto. apply dlCons ; auto.
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    clear e. rewrite unBox_app_distrib in x. pose (G4iSL_adm_wkn_L x H0). auto.
Qed.
