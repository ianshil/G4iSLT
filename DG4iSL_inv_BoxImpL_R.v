...
Theorem BoxImpL_hpinv_R : forall (k : nat) concl
        ...
        k = (derrec_height D0) ->
          ...
          existsT2 (D1 : derrec G4iSL_rules (fun _ => False) prem2),
          derrec_height D1 <= k))).
Proof.
...
- intros hei. intros prem1 prem2 RA. inversion RA. subst.
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
     ...
     pose (G4iSL_hpadm_list_exch_L _ _ _ J50 _ l). destruct s.
  ...
  * inversion H. subst. apply list_split_form in H2. destruct H2. repeat destruct s ; repeat destruct p ; subst.
   ...
   + repeat destruct s. repeat destruct p ; subst.
      ...
      pose (G4iSL_hpadm_list_exch_L _ _ _ J50 _ J51). destruct s. simpl. repeat rewrite <- app_assoc. simpl. repeat rewrite <- app_assoc.
...
Theorem BoxImpL_inv_R : forall concl prem1 prem2, (derrec G4iSL_rules (fun _ => False) concl) ->
