...
Theorem unBox_left_hpadm0 : forall (k : nat) s
        ...
        (forall A Γ0 Γ1 C, (s = (Γ0 ++ Box A :: Γ1, C)) ->
          existsT2 (D1 : derrec G4iSL_rules (fun _ => False) (Γ0 ++ A :: Γ1, C)),
          derrec_height D1 <= k).
Proof.
...
- intros hei A Γ0 Γ1 C eq. subst. inversion g ; subst.
  ...
  * inversion H. subst. simpl. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    pose (G4iSL_hpadm_list_exch_L _ _ _ J6 _ H0). destruct s.
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    + repeat destruct s ; repeat destruct p ; subst.
       ...
       pose (G4iSL_hpadm_list_exch_L _ _ _ J8 _ l0). destruct s. pose (dlCons x3 DersNilF). pose (derI _ o d0). exists d1.
  ...
  * inversion H. subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    + assert (BoxImpLRule [(unBoxed_list (Γ0 ++ A :: x2) ++ B :: unBoxed_list Γ3 ++ [Box A0], A0);(Γ0 ++ A :: x2 ++ B :: Γ3, C)] (Γ0 ++ A :: x2 ++ Box A0 → B :: Γ3, C)).
       ...
       assert (existsT2 D1 : derrec G4iSL_rules (fun _ : Seq => False) (unBoxed_list Γ0 ++ unBox_formula A :: unBoxed_list x2 ++ B :: unBoxed_list Γ3 ++ [Box A0], A0),
    ...
    + repeat destruct s ; repeat destruct p ; subst.
       ...
       assert (J10: (Γ2 ++ B :: x1 ++ Box A :: Γ1, C) = ((Γ2 ++ B :: x1) ++ Box A :: Γ1, C)). repeat rewrite <- app_assoc ; simpl ; auto.
       pose (IH _ J8 _ _ J9 _ _ _ _ J10). destruct s. simpl in IH.
       assert (existsT2 D1 : derrec G4iSL_rules (fun _ : Seq => False) (unBoxed_list Γ2 ++ B :: (unBoxed_list x1 ++ unBox_formula A :: unBoxed_list Γ1) ++ [Box A0], A0),
       derrec_height D1 <= derrec_height x).
       { destruct (dec_is_box A).
         -- destruct s ; subst. simpl.
  ...
  * inversion H ; subst. assert (J30: dersrec_height d = dersrec_height d). reflexivity.
    ...
    assert (J5: derrec_height x < S (dersrec_height d)). lia. assert (J6: derrec_height x = derrec_height x). reflexivity.
    assert (existsT2 D1 : derrec G4iSL_rules (fun _ : Seq => False) (unBoxed_list Γ0 ++ unBox_formula A :: unBoxed_list Γ1 ++ [Box A0], A0),
    derrec_height D1 <= derrec_height x).
    { destruct (dec_is_box A).
      -- destruct s ; subst. simpl.
...
Theorem unBox_left_hpadm : forall A Γ0 Γ1 C (D: G4iSL_prv (Γ0 ++ Box A :: Γ1, C)),
        existsT2 (D0: G4iSL_prv (Γ0 ++ A :: Γ1, C)), derrec_height D0 <= derrec_height D.
Proof.
intros.
...
Theorem unBox_left_hpadm_gen : forall A Γ0 Γ1 C (D: G4iSL_prv (Γ0 ++ A :: Γ1, C)),
        existsT2 (D0: G4iSL_prv (Γ0 ++ unBox_formula A :: Γ1, C)), derrec_height D0 <= derrec_height D.
Proof.
...
Theorem unBox_left_adm : forall A Γ0 Γ1 C, (G4iSL_prv (Γ0 ++ Box A :: Γ1, C)) -> (G4iSL_prv (Γ0 ++ A :: Γ1, C)).
Proof.
intros.
apply unBox_left_hpadm in X ; auto.
Qed.

Theorem unBox_left_adm_gen : forall A Γ0 Γ1 C, (G4iSL_prv (Γ0 ++ A :: Γ1, C)) -> (G4iSL_prv (Γ0 ++ unBox_formula A :: Γ1, C)).
Proof.
intros.
apply unBox_left_hpadm_gen in X ; auto.
Qed.

Theorem unBoxed_list_left_adm_gen : forall Γ1 Γ0 Γ2 C, (G4iSL_prv (Γ0 ++ Γ1 ++ Γ2, C)) -> (G4iSL_prv (Γ0 ++ unBoxed_list Γ1 ++ Γ2, C)).
Proof.
