...
Require Import DG4iSL_termination_measure_All.
Require Import DG4iSL_exch.
Require Import DG4iSL_wkn.
Require Import DG4iSL_Id.
Require Import DG4iSL_inv_ImpL_R.
...
Theorem G4iSL_cut_adm_main : forall n A s Γ0 Γ1 C,
                      n = weight_form A ->
                      (s = (Γ0 ++ Γ1, C)) ->
                      (derrec G4iSL_rules (fun _ => False) (Γ0 ++ Γ1, A)) ->
                      (derrec G4iSL_rules (fun _ => False) (Γ0 ++ A :: Γ1, C)) ->
...
pose (less_thanS_strong_inductionT (fun s => forall A (Γ0 Γ1 : list MPropF) (C : MPropF),
                  n = weight_form A ->
                  s = (Γ0 ++ Γ1, C) ->
                  derrec G4iSL_rules (fun _ : Seq => False) (Γ0 ++ Γ1, A) ->
                  derrec G4iSL_rules (fun _ : Seq => False) (Γ0 ++ A :: Γ1, C) -> derrec G4iSL_rules (fun _ : Seq => False) s)).
...
apply dersrec_nil.
assert (DersNilT: dersrec G4iSL_rules (fun _ : Seq => True) []).
apply dersrec_nil.
...
(* Left rule is IdP *)
- inversion H3. subst.
  pose (list_exch_LI [] [] Γ0  [# P] Γ1 C). simpl in l. rewrite <- H4 in l. pose (G4iSL_adm_list_exch_L _ D1 _ l).
  pose (ctr_LI (# P) [] Γ2 Γ3 C). simpl in c. pose (G4iSL_adm_ctr_L d c).
  pose (list_exch_LI [] [# P] Γ2  [] Γ3 C). simpl in l0. pose (G4iSL_adm_list_exch_L _ d0 _ l0). auto.
...
- inversion H3. subst. inversion X. subst. clear X2. clear X0.
  pose (list_exch_LI [] [] Γ0  [A] Γ1 C). simpl in l. rewrite <- H4 in l. pose (G4iSL_adm_list_exch_L _ D1 _ l).
  ...
  pose (G4iSL_less_thanS _ _ J1 _ J2). rewrite <- H4 in SIH.
...
- inversion H3. subst. inversion X. subst. inversion X2. subst. clear X4. clear X2. clear X. clear X0.
  ...
  pose (G4iSL_less_thanS _ _ J1 _ J6).
...
- inversion H3. subst. inversion X. subst. clear X2. clear X. inversion H1. simpl.
    ...
    { inversion H0. subst. inversion X0. subst. inversion X2. subst. rewrite H4. clear X4. clear X2.
      ...
      pose (G4iSL_less_thanS _ _ H2 _ J21).
      ...
      pose (G4iSL_less_thanS _ _ H2 _ J31).
    ...
    { inversion H0. subst. inversion X0. subst. inversion X2. subst. clear X4. clear X2. clear X0.
      ...
      - repeat destruct s. repeat destruct p. subst.
        ...
        pose (G4iSL_less_thanS _ _ H2 _ J22).
    ...
    { inversion H0. subst. inversion X0. subst. clear X2. rewrite <- H8 in D1. rewrite H4.
      pose (list_exch_LI [] [] Γ4 [A] Γ5 B0). simpl in l. rewrite H8 in l. pose (G4iSL_adm_list_exch_L _ X _ l).
      ...
      pose (@G4iSL_adm_wkn_L (Γ0 ++ Γ1, A0 → B) D0 (A :: Γ0 ++ Γ1, A0 → B) A H5).
    ...
    { inversion H0. subst. inversion X0. subst. clear X2. clear X0. apply list_split_form in H8. destruct H8 ; subst. destruct s.
      - repeat destruct p. inversion e0. subst. rewrite H4.
        pose (list_exch_LI [] [] Γ2 [A ∧ B0] Γ3 B). simpl in l. pose (G4iSL_adm_list_exch_L _ X1 _ l). rewrite H4 in d.
        assert (J1: AndLRule [([] ++ A :: B0 :: Γ0 ++ Γ1, B)] ([] ++ A ∧ B0 :: Γ0 ++ Γ1, B)). apply AndLRule_I. simpl in J1. pose (AndL_inv _ _ d J1).
        assert (derrec G4iSL_rules (fun _ : list (MPropF) * MPropF => False) ([] ++ Γ0 ++ Γ1, A → B0 → B)).
    ...
    { inversion H0. subst. inversion X0. subst. clear X2. clear X0. apply list_split_form in H8. destruct H8 ; subst. destruct s.
      - repeat destruct p. inversion e0. subst. rewrite H4.
        pose (list_exch_LI [] [] Γ2 [A ∨  B0] Γ3 B). simpl in l. rewrite H4 in l. pose (G4iSL_adm_list_exch_L _ X1 _ l).
        ...
        assert (derrec G4iSL_rules (fun _ : list (MPropF) * MPropF => False) ([] ++ Γ0 ++ Γ5 ++ Γ6, A → B)).
        apply derI with (ps:=[([] ++ A :: Γ0 ++ Γ5 ++ Γ6, B)]). apply ImpR. apply ImpRRule_I. apply dlCons ; auto. simpl in X0.
        assert (derrec G4iSL_rules (fun _ : list (MPropF) * MPropF => False) ([] ++ Γ0 ++ Γ5 ++ Γ6, B0 → B)).
      ...
      - repeat destruct s. repeat destruct p. subst. repeat rewrite <- app_assoc. simpl. rewrite H4. repeat rewrite <- app_assoc. simpl.
        ...
        pose (G4iSL_adm_list_exch_L (Γ4 ++ A → C0 :: Γ5 ++ B0 → C0 :: Γ6, C) X (Γ4 ++ A → C0 :: B0 → C0 :: x ++ A0 → B :: Γ1, C) l0).
        pose (list_exch_LI Γ4 [] (A → C0 :: B0 → C0 :: x) [A0 → B] Γ1 C). simpl in l1. repeat rewrite <- app_assoc in l1. simpl in l1.
        pose (G4iSL_adm_list_exch_L _ d2 _ l1). pose (d1 d3). apply derI with (ps:=[(Γ4 ++ A → C0 :: B0 → C0 :: x ++ Γ1, C)]).
    ...
    { inversion H0. subst. inversion X0. subst. inversion X2. subst. clear X4. clear X2. clear X0.
      ...
      - repeat destruct p. inversion e0. subst. rewrite H4.
        ...
        assert (derrec G4iSL_rules (fun _ : list (MPropF) * MPropF => False) ([] ++ Γ0 ++ Γ1, B0 → B)).
      ...
      - repeat destruct s. repeat destruct p. subst. rewrite H4.
        ...
        pose (@G4iSL_adm_ctr_L _ d _ _ c).
        ...
        pose (G4iSL_less_thanS _ _ J33 _ J34).
        assert (J5: weight_form (A0 → B) = weight_form (A0 → B)). reflexivity.
      ...
      - repeat destruct s. repeat destruct p. subst. repeat rewrite <- app_assoc. simpl. rewrite H4.
        ...
        repeat rewrite <- app_assoc ; simpl ; auto.
        pose (ImpImpL_inv_L _ _ _ _ _ _ _ _ _ J60 J61).
        pose (ctr_LI (B0 → C0) (Γ4 ++ [A]) [] (x ++ Γ1) (A0 → B)). simpl in c. repeat rewrite <- app_assoc in c ; simpl in c. pose (@G4iSL_adm_ctr_L _ d _ _ c).
    ...
    { inversion H0 ; subst. inversion X0 ; subst. inversion X2 ; subst. clear X4. clear X2. clear X0. rewrite H4.
      ...
      - repeat destruct p ; subst. inversion e0 ; subst.
        ...
        pose (list_exch_LI [] [] Γ2 [Box A] Γ3 B). simpl in l. rewrite H4 in l. pose (G4iSL_adm_list_exch_L _ X1 _ l).
        pose (list_exch_LI [] [Box A] (Γ0 ++ Γ1) [] [] B). simpl in l0. repeat rewrite <- app_assoc in l0. rewrite <- app_nil_end in l0.
        pose (G4iSL_adm_list_exch_L _ d _ l0). auto.
        ...
        pose (list_exch_LI [] [] Γ2 [Box A] Γ3 B). simpl in l. rewrite H4 in l. pose (G4iSL_adm_list_exch_L _ X1 _ l).
        pose (list_exch_LI [] [Box A] Γ0 [] Γ1 B). simpl in l0. pose (G4iSL_adm_list_exch_L _ d _ l0). auto.
      - repeat destruct s ; repeat destruct p ; subst. repeat rewrite <- app_assoc in X3 ; simpl in X3.
        ...
        repeat rewrite <- app_assoc ; apply InT_eq. pose (G4iSL_less_thanS _ _ J0 _ J21).
        assert (J5: weight_form (A0 → B) = weight_form (A0 → B)). reflexivity.
        ...
        pose (G4iSL_less_thanS _ _ J0 _ J22).
        ...
        apply dlCons ; auto. pose (list_exch_LI [] [] Γ2 [A0] Γ3 B). simpl in l1. rewrite H4 in l1. pose (G4iSL_adm_list_exch_L _ X1 _ l1).
        ...
        pose (G4iSL_adm_wkn_L d3 w). simpl in d4 ; repeat rewrite <- app_assoc in d4.
      ...
      - repeat destruct s ; repeat destruct p ; subst. repeat rewrite <- app_assoc. simpl.
        ...
        pose (list_exch_LI [] [] Γ2 [A0] Γ3 B). simpl in l1. rewrite H4 in l1. pose (G4iSL_adm_list_exch_L _ X1 _ l1). repeat rewrite <- app_assoc in d2. simpl in d2.
        assert (J80: derrec_height d2 = derrec_height d2). auto.
        ...
        pose (G4iSL_adm_wkn_L d3 w). simpl in d4; repeat rewrite <- app_assoc in d4 ; simpl in d4 ; repeat rewrite <- app_assoc in d4 ; auto.
    ...
    { inversion H0. subst. rewrite H4. inversion X0. subst. clear X2. clear X0.
      ...
      pose (list_exch_LI [] [] Γ2 [A0] Γ3 B). simpl in l0. rewrite H4 in l0. pose (G4iSL_adm_list_exch_L _ X1 _ l0).
      ...
      epose (G4iSL_adm_wkn_L g0 w). simpl in d1 ; repeat rewrite <- app_assoc in d1 ; auto.
      apply DersNilF. }
...
- inversion H3. subst. inversion H1.
    ...
    { inversion H0. subst. inversion X0. subst. clear X2. rewrite <- H7 in D1.
      pose (list_exch_LI [] [] Γ2 [A] Γ3 B). simpl in l. rewrite H7 in l. pose (G4iSL_adm_list_exch_L _ X1 _ l).
      ...
      pose (@G4iSL_adm_wkn_L (Γ0 ++ Γ1, Box A0) D0 (A :: Γ0 ++ Γ1, Box A0) A H4).
    ...
    { inversion H0. subst. inversion X0. subst. clear X2. clear X0. apply list_split_form in H7. destruct H7 ; subst. destruct s.
      ...
      - repeat destruct s. repeat destruct p. subst. repeat rewrite <- app_assoc. simpl.
        ...
        pose (G4iSL_adm_list_exch_L _ X1 _ l0). pose (d1 d2). apply derI with (ps:=[(Γ2 ++ A → C0 :: B → C0 :: x ++ Γ1, C)]).
    ...
    { inversion H0. subst. inversion X0. subst. inversion X2. subst. clear X4. clear X2. clear X0. apply list_split_form in H7. destruct H7 ; subst. destruct s.
      ...
      - repeat destruct s. repeat destruct p. subst.
        ...
        pose (@G4iSL_adm_ctr_L _ d _ (B → C0) c).
      ...
      - repeat destruct s. repeat destruct p. subst. repeat rewrite <- app_assoc. simpl.
        ...
        pose (ImpImpL_inv_L _ _ _ _ _ _ _ _ _ J60 J61).
        pose (ctr_LI (B → C0) (Γ2 ++ [A]) [] (x ++ Γ1) (Box A0)). simpl in c. repeat rewrite <- app_assoc in c ; simpl in c. pose (@G4iSL_adm_ctr_L _ d _ _ c).
    ...
    { inversion H0. subst. inversion X0. subst. clear X0. inversion X2. subst. inversion X. subst. clear X5. clear X3. clear X.
      ...
      - repeat destruct s ; repeat destruct p ; subst. rewrite <- app_assoc in X0 ; simpl in X0.
         ...
         pose (G4iSL_less_thanS _ _ J0 _ J21).
         assert (J5: weight_form (Box A0) = weight_form (Box A0)). reflexivity.
         ...
         pose (G4iSL_adm_wkn_L d w). simpl in d0 ; repeat rewrite <- app_assoc in d0 ; simpl in d0 ; auto.
         ...
         apply G4iSL_adm_list_exch_L with (s:=(unBoxed_list Γ0 ++ unBoxed_list x0 ++ B :: unBoxed_list Γ3 ++ [Box A0; Box A], A0)) ; auto.
         ...
         apply G4iSL_adm_wkn_L with (A:=Box A0) (s:=(unBoxed_list Γ0 ++ A0 :: unBoxed_list x0 ++ B :: unBoxed_list Γ3 ++ [Box A], A)) ; auto.
      ...
      - repeat destruct s ; repeat destruct p ; subst. rewrite <- app_assoc. simpl.
         ...
         pose (G4iSL_adm_wkn_L d w). simpl in d0 ; repeat rewrite <- app_assoc in d0 ; simpl in d0 ; auto. repeat rewrite <- app_assoc in d0.
         ...
         apply G4iSL_adm_list_exch_L with (s:=(unBoxed_list Γ2 ++ B :: unBoxed_list x ++ unBoxed_list Γ1 ++ [Box A0; Box A], A0)) ; auto.
         repeat rewrite unBox_app_distrib in X1 ; repeat rewrite <- app_assoc in X1 ; simpl in X1.
         pose (wkn_LI (Box A0) (unBoxed_list Γ2 ++ [A0]) (B :: unBoxed_list x ++ unBoxed_list Γ1 ++ [Box A]) A).
         simpl in w ; repeat rewrite <- app_assoc in w ; simpl in w ; repeat rewrite <- app_assoc in w ; simpl in w.
         apply G4iSL_adm_wkn_L with (A:=Box A0) (s:=(unBoxed_list Γ2 ++ A0 :: B :: unBoxed_list x ++ unBoxed_list Γ1 ++ [Box A], A)) ; auto.
         ...
         apply G4iSL_adm_list_exch_L with (s:=(unBoxed_list Γ2 ++ B :: unBoxed_list x ++ A0 :: unBoxed_list Γ1 ++ [Box A], A)) ; auto.
         ...
         apply G4iSL_adm_list_exch_L with (s:=(Γ2 ++ B :: x ++ Box A0 :: Γ1, C)) ; auto. }
   (* Right rule is SLR *)
    { inversion H0 ; subst. inversion X0. subst. clear X2. inversion X. subst. clear X3. clear X0. clear X.
      ...
      apply G4iSL_adm_wkn_L with (A:=Box A) (s:=(unBoxed_list Γ0 ++ unBoxed_list Γ1, Box A0)) ; auto.
      ...
      apply G4iSL_adm_list_exch_L with (s:=(unBoxed_list Γ0 ++ Box A :: unBoxed_list Γ1 ++ [Box A0], A0)) ; auto.
      pose (wkn_LI (Box A) (unBoxed_list Γ0) (unBoxed_list Γ1 ++ [Box A0]) A0). simpl in w ; repeat rewrite <- app_assoc in w.
      apply G4iSL_adm_wkn_L with (A:=Box A) (s:=(unBoxed_list Γ0 ++ unBoxed_list Γ1 ++ [Box A0], A0)) ; auto.
      ...
      apply G4iSL_adm_wkn_L with (A:=Box A0) (s:=(unBoxed_list Γ0 ++ A0 :: unBoxed_list Γ1 ++ [Box A], A)) ; auto. }
Qed.

Theorem G4iSL_cut_adm0 : forall A Γ0 Γ1 C,
                      G4iSL_prv (Γ0 ++ Γ1, A) ->
                      G4iSL_prv (Γ0 ++ A :: Γ1, C) ->
                      G4iSL_prv (Γ0 ++ Γ1, C).
Proof.
...
pose (@G4iSL_cut_adm_main (weight_form A) A
(Γ0 ++ Γ1, C) Γ0 Γ1 C J1 J3). unfold G4iSL_prv. auto.
Qed.

Theorem G4iSL_cut_adm : forall A Γ0 Γ1 C,
                      (G4iSL_prv (Γ0 ++ Γ1, A) * G4iSL_prv (Γ0 ++ A :: Γ1, C)) ->
                      G4iSL_prv (Γ0 ++ Γ1, C).
Proof.
intros. destruct X. apply G4iSL_cut_adm0 with (A:=A) ; auto.
Qed.